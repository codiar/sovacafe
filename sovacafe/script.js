        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    heroSub.style.transition = 'all 1s ease 0.3s';
...
// --- 2. MENU DATA ---
const WHATSAPP_NUMBER = '9647892966486'; // ضع الرقم الصحيح هنا
...
function getWhatsAppLink(item) {
    const text =
        `مرحباً SOVA CAFE،%0Aأود طلب:%0A☕ ${item.nameAr} (${item.nameEn})%0A💰 السعر: ${item.price} دينار%0Aالكمية: [أدخل الكمية هنا]`;
...
const revealElements = document.querySelectorAll('.reveal:not(.menu-card)');
